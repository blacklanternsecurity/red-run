...
- [x] `network-recon` (880 lines) — Discovery skill. Passive recon (DNS, Shodan, cert transparency), host discovery (ARP/ICMP/TCP/UDP sweeps), port scanning (default: `nmap -A -p- -T4 -oA -vvv`, plus stealth/evasion variants: decoys, fragmentation, source port spoofing, idle scan, timing control, masscan for speed), service enumeration for 20+ protocols (FTP/SSH/SMTP/DNS/HTTP/Kerberos/RPC/SMB/LDAP/MSSQL/Oracle/MySQL/RDP/PostgreSQL/WinRM/Redis/MongoDB/SNMP/IPMI/NFS/TFTP/VNC — each with embedded quick-win checks: anonymous access, default creds, version CVEs, RCE techniques), OS fingerprinting (TTL heuristics, port signature patterns), vulnerability scanning (NSE vuln scripts, nuclei templates), multi-host pipeline workflows, output parsing for state.md population, routing decision tree to web-discovery/ad-discovery/technique skills. Bash fallback scanning for pivots without nmap.
- [x] `pivoting-tunneling` (1108 lines) — Tool selection decision tree by scenario. SSH tunneling (local -L, remote -R, dynamic SOCKS -D, escape sequence ~C, ProxyJump -J multi-hop, VPN TUN device), sshuttle (transparent subnet routing, DNS forwarding, no proxychains needed), Ligolo-ng (TUN interface, agent/proxy setup, listener port forwarding, double pivot via listener chaining), Chisel (reverse SOCKS, port forward, HTTP proxy support, TLS mode), socat (port relay, SSL tunnels, UDP relay, reverse shell relay), Windows pivoting (netsh portproxy, plink, SocksOverRDP), DNS tunneling (dnscat2, iodine TUN), ICMP tunneling (hans, ptunnel-ng), HTTP tunneling (neo-reGeorg webshell SOCKS, rpivot NTLM proxy bypass), Metasploit pivoting (autoroute, portfwd, SOCKS module), FRP (config-file based reverse proxy), ngrok/cloudflared (public tunnels for CTF), multi-hop chaining (SSH ProxyJump, Ligolo double pivot, Chisel chain, proxychains strict_chain), tunnel verification procedures, SOCKS tool compatibility table (14 tools), persistent tunnel management (autossh, keep-alive, tmux). Python minimal SOCKS4 proxy fallback when no tools available.

**Batch 2: Containers** — container escape and orchestrator exploitation
- [x] `container-escapes` (1021 lines) — Container detection and enumeration (type detection, capability check, automated tools: deepce/CDK/amicontained), Docker socket escape (CLI and curl API, containerd socket), privileged container escape (host fs mount, nsenter, cgroup release_agent with CVE-2022-0492, kernel module loading), sensitive mount exploitation (/proc/sys/kernel/core_pattern, uevent_helper, modprobe, host volume mounts), capability-specific escapes (cap_sys_ptrace injection, cap_dac_read_search Shocker, cap_sys_module, cap_net_admin), remote Docker API exploitation (2375/2376), Kubernetes SA token exploitation (API access, permission enumeration, secret extraction, pod inspection, malicious pod creation with BadPods), kubelet API abuse (10250/10255, pod exec), etcd secret extraction (direct API, etcdctl, curl), RBAC exploitation (wildcard perms, SA impersonation, RoleBinding escalation), container CVEs (CVE-2019-5736 runc, CVE-2022-0492 cgroup, CVE-2024-21626 Leaky Vessels, CVE-2024-1753 Buildah, CVE-2025-31133 runc race), cloud metadata from containers (AWS IMDSv1/v2, GCP, Azure), routing decision tree
...
- [ ] Cloud — AWS (IAM, IMDS, S3, Lambda, EC2 role abuse)
- [ ] Cloud — Azure (Azure AD, tokens, CAP bypass, service abuse, managed identity)
- [ ] CI/CD (GitHub Actions, GitLab CI, secrets extraction, pipeline poisoning)
