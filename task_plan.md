...
- [ ] `windows-uac-bypass` — UAC bypass + COM hijacking + MSI abuse + autorun exploitation.
...
- [ ] `windows-credential-harvesting` — Local credential discovery (distinct from Phase 4 `credential-dumping` which covers AD-level DCSync/NTDS/LAPS/gMSA).
...
- [ ] `windows-kernel-exploits` — Kernel CVEs, BYOVD, privileged file operations, named pipes, leaked handles.
