...
- **Activity** → append to `engagement/activity.md` at milestones (test confirmed,
  ...
  `### [YYYY-MM-DD HH:MM:SS] sql-injection-blind → <target>` with bullet points of actions/results.
...
- **Evidence** → save significant output to `engagement/evidence/` with descriptive
  filenames (e.g., `sqli-blind-extracted-data.txt`).
...
1. **On-screen**: Print `[sql-injection-blind] Activated → <target>` so the operator
   sees which skill is running.
2. **activity.md**: Append:
   ```
   ### [YYYY-MM-DD HH:MM:SS] sql-injection-blind → <target>
