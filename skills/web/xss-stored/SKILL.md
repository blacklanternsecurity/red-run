...
- **Activity** → append to `engagement/activity.md` at milestones (test confirmed,
  stored payload triggered, blind callback received, pivot to another skill):
  `### [HH:MM] xss-stored → <target>` with bullet points of actions/results.
...
- **Evidence** → save significant output to `engagement/evidence/` with descriptive
  filenames (e.g., `xss-stored-callback-capture.txt`).
...
Read $RED_RUN_DOCS/hacktricks/src/pentesting-web/xss-cross-site-scripting/server-side-xss-dynamic-pdf.md
```
