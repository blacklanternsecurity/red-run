...
- **Activity** → append to `engagement/activity.md` at milestones (CSRF confirmed,
  PoC built, impact demonstrated):
  `### [HH:MM] csrf → <target>` with bullet points of actions/results.
...
- **Evidence** → save significant output to `engagement/evidence/` with descriptive
  filenames (e.g., `csrf-email-change-poc.html`, `csrf-token-bypass.txt`).
