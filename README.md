...
| Repo | Strength |
|------|----------|
| [PayloadsAllTheThings](https://github.com/swisskyrepo/PayloadsAllTheThings) | Web payloads, injection techniques — deepest per-technique coverage |
...
These skills are a **baseline** built from three public source repositories (PayloadsAllTheThings, InternalAllTheThings, HackTricks). They cover the most common techniques with the top 2-3 payloads per variant. Nearly all skill content was generated by Claude and has not been thoroughly human-reviewed — treat it as a starting point, not a verified reference. Expect errors, gaps, and techniques that need validation against real targets.
...
Phase 6 (Infrastructure & Network) complete. 60 skills built, ~32,400 lines. See `task_plan.md` for the full build plan.
