...
|---------|-------------|-------|--------------|
| Full Pentest | All phases enabled (default, via freeform) | none | ask |
...
Full Pentest is the default — select it via the freeform input. Profiles also support model hints (e.g., upgrade web-exploit-agent to Opus for complex targets) and priority phase ordering for parallel path recommendations. See `skills/orchestrator/templates/` for the full definitions.
