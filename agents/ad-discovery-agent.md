...
**Phase grouping for AD discovery:**
- Unauthenticated enumeration (null sessions, RID cycling, anonymous LDAP
  binds) = one phase
- BloodHound collection (authenticated, generates significant AD traffic) = one
  phase
- Targeted enumeration (SPNs, delegation, ADCS, ACLs) = one phase
