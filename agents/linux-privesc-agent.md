...
**Phase grouping for Linux privesc:**
- Host enumeration (system info, users, network, running processes) = one phase
- Privilege escalation attempt (sudo abuse, SUID exploit, kernel exploit) = one
  phase
- Post-exploitation (credential harvesting, evidence collection) = one phase
