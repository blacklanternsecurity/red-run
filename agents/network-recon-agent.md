...
**Phase grouping for network recon:**
- Port scan (single nmap invocation) = one phase
- Service enumeration + unauthenticated quick-wins (null sessions, anonymous
  binds, banner grabs) = one phase
- Authenticated enumeration with discovered credentials = one phase
- Vulnerability scanning (NSE scripts, nuclei) = one phase
